...
export const TicketManagement = () => {
      ...
      setTickets(prev =>
        prev.map(ticket =>
          ticket.id === selectedTicket.id
            ? { ...ticket, status: "closed", lastUpdate: new Date().toISOString() }
            : ticket
        )
      );
  ...
  const pendingReviewTickets = tickets.filter(t => t.status === "pending-review").length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-green-600 dark:text-green-400">Closed</p>
          ...
          {selectedTicket && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div><strong>Request Type:</strong> {selectedTicket.requestType.replace("-", " ")}</div>
                    {selectedTicket.jobTitle && (
                      <div><strong>Job Title:</strong> {selectedTicket.jobTitle}</div>
                    )}
                  </div>
                </div>
                
                <div>
                  <h4 className="font-medium text-foreground mb-2">Ticket Status</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                    ...
                    <div><strong>Staff:</strong> {selectedTicket.staffAssigned}</div>
                  </div>
                </div>
              </div>
              ...
              {selectedTicket.candidatesFound && (
                <div>
                  <h4 className="font-medium text-foreground mb-2">Progress Update</h4>
                  <div className="p-3 bg-green-50 dark:bg-green-950/20 rounded-lg text-sm">
                    <div className="flex items-center gap-2 text-success">
                      <CheckCircle className="w-4 h-4" />
                      <strong>{selectedTicket.candidatesFound} candidates found</strong>
                    </div>
                    {selectedTicket.estimatedCompletion && (
                      <div className="mt-2 text-muted-foreground">
                        Estimated completion: {new Date(selectedTicket.estimatedCompletion).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
                <div>
                  <strong>Created:</strong> {new Date(selectedTicket.createdDate).toLocaleString()}
                </div>
                <div>
                  <strong>Last Update:</strong> {new Date(selectedTicket.lastUpdate).toLocaleString()}
                </div>
              </div>
            </div>
          )}
      ...
      <Dialog open={isCloseModalOpen} onOpenChange={setIsCloseModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for closing this ticket. This action will update the personnel platform.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-foreground">Closure Reason</label>
              <Textarea
                value={closeReason}
